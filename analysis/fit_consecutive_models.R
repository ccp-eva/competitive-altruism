...
m_second_proposer_raises_by_prev <- brm(outbids ~ 1 + previously_accepted*session +
		...
		prior = prior(normal(0, 1.5), class="Intercept") +
			...
			prior(normal(0, 0.75/15), class="b", coef="session") +
			prior(normal(0, 0.75/15), class="b", coef="previously_accepted:session"),
...
# Un-center session values for simplicity below
d$session <- d$session + 8.5
for(i in 1:16) {
...
# baselines
nd <- expand_grid(first_offer=0:8, previously_accepted=c(-0.5, 0.5), session=1:16 - 8.5) %>%
	left_join(d_r)
