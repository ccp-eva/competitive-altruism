...
preds <- posterior_predict(m2, newdata=nd, re_formula=NA)
plot_table <- tibble(game_type=character(),
		     total_offer_prop=numeric())
for(i in 1:nrow(preds)) {
	plot_table <- add_row(plot_table, game_type="dyadic", total_offer_prop=preds[i,1])
	plot_table <- add_row(plot_table, game_type="triadic_con", total_offer_prop=preds[i,2])
	plot_table <- add_row(plot_table, game_type="triadic_sim", total_offer_prop=preds[i,3])
}
